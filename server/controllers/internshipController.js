...
exports.getAllInternships = async (req, res) => {
    try {
        ...
        if (req.query.compensation) {
            filter.compensation = req.query.compensation;
        }
        ...
        if (req.query.minStipend || req.query.maxStipend) {
            ...
            if (req.query.minStipend) {
                filter.stipend.$gte = parseInt(req.query.minStipend);
            }
            
            if (req.query.maxStipend) {
                filter.stipend.$lte = parseInt(req.query.maxStipend);
            }
        }
        
        // Search by text (title, tags, skills, company name)
        if (req.query.search) {
        ...
        // Filter by specific tag or skill
        if (req.query.tag) {
            filter.$or = [
                { tags: req.query.tag },
                { skills: req.query.tag }
            ];
        }
        ...
        const internships = await Internship.find(filter)
            ...
            .skip(skip)
            .limit(limit)
            .populate('company', 'name email organizationName');
...
exports.getInternshipById = async (req, res) => {
    try {
        const internship = await Internship.findById(req.params.id)
            .populate('company', 'name email organizationName website description');
