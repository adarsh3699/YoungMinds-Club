...
const EventCard: React.FC<EventCardProps> = ({
	...
	const handleSaveToggle = (e: React.MouseEvent<HTMLButtonElement>): void => {
		e.preventDefault();
		e.stopPropagation();
	...
	const categoryColorGradient = getCategoryColor(event.type || event.category || 'default');
	...
	const getLocationDisplay = (): string => {
		...
		if (event.location?.city && event.location?.venue) {
			return `${event.location.city}, ${event.location.venue}`;
		}
		return event.venue || 'TBA';
	};

	// Get registration stats
	const registrationCount = event.registrationCount || event.currentAttendees || 0;
		...
		>
			...
			<div className="ym-bg-card rounded-xl overflow-hidden shadow-md hover:shadow-xl transition-all duration-300 flex flex-col h-full border ym-border-card">
				...
				<Link to={`/event/${event.id || event._id}`} className="block relative h-52 overflow-hidden">
					<img
						src={event.poster || event.eventImage || 'https://via.placeholder.com/400x200?text=No+Image'}
						alt={event.title}
				...
				<div className="p-4 flex-grow flex flex-col">
					...
					<p className="ym-text-secondary text-sm mb-3 line-clamp-2">
					...
					<div className="mt-auto">
						...
						<div className="flex justify-between text-xs ym-text-muted mb-3">
							...
							<span>{capacity - registrationCount} spots left</span>
						</div>
