...
import { MagnifyingGlassIcon, ChevronDownIcon, ChevronUpIcon, TagIcon, AdjustmentsHorizontalIcon, CalendarIcon, MapPinIcon, UserGroupIcon, CurrencyDollarIcon } from '@heroicons/react/24/outline';
...
const searchAndFillter: React.FC<EnhancedSearchFiltersProps> = memo(({
	...
	const hasActiveAdvancedFilters = advancedFilters && (
						...
						<div className="px-8 py-6 space-y-6 border-t border-border/20 bg-gradient-to-br from-muted/5 via-transparent to-accent/5 animate-fade-in-up">
								...
								<div>
									<div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
										<DateTimePicker
											...
											onChange={(e) => advancedFilters.dateRange?.setStartDate(e.target.value)}
										...
										<DateTimePicker
											...
											className="w-full"
											minDate={advancedFilters.dateRange.startDate ? new Date(advancedFilters.dateRange.startDate) : undefined}
											showTimeSelect={false}
										/>
									</div>
								</div>
							...
							<div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
								...
								{advancedFilters.toggleFilters && (
									<div>
										<label className="block text-s font-semibold text-card-foreground mb-3">Quick Filters</label>
										<div className="space-y-3">
											{advancedFilters.toggleFilters.isOnlineOnly && (
												<Switch
													enabled={advancedFilters.toggleFilters.isOnlineOnly.value}
													onChange={(event) => advancedFilters.toggleFilters?.isOnlineOnly?.setValue(event.target.checked)}
													label="Online Only"
												/>
											)}
											{advancedFilters.toggleFilters.isFeaturedOnly && (
												<Switch
													...
													onChange={(event) => advancedFilters.toggleFilters?.isFeaturedOnly?.setValue(event.target.checked)}
													label="Featured Only"
												/>
											)}
										</div>
								...
								{advancedFilters.registrationRange && (
									<>
										<FormInput
											...
											onChange={(e) => advancedFilters.registrationRange?.setMin(e.target.value)}
											className="w-full"
											min="0"
											allowNegative={false}
										/>
										<FormInput
											...
											onChange={(e) => advancedFilters.registrationRange?.setMax(e.target.value)}
											className="w-full"
											min="0"
											allowNegative={false}
										/>
									...
									<div className="flex items-center justify-between">
										<div className="flex items-center gap-2">
											...
											<span className="text-sm font-medium text-card-foreground">
												<span className="text-primary font-semibold">{filteredCount}</span> of {totalCount} {itemType} shown
											</span>
										</div>
										<button
