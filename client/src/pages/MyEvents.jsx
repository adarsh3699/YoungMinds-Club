  ...
  useEffect(() => {
    const fetchLeaderboard = async () => {
      try {
        ...
        // Validate and sanitize the leaderboard data
        const sanitizedData = response.data.leaderboard.map(item => ({
          ...item,
          user: item.user || { name: 'Unknown User' },
          xp: item.xp || 0,
          badge: item.badge || 'Newbie',
              ...
              <tr key={index} className={index < 3 ? 'bg-yellow-50 dark:bg-yellow-900/20' : ''}>
                ...
                <td className="px-6 py-4 whitespace-nowrap">
                  <div className="flex items-center">
                    ...
                    <div className="ml-4">
                      <div className="text-sm font-medium">{item.user?.name || 'Unknown User'}</div>
                    </div>
                  </div>
                </td>
                <td className="px-6 py-4 whitespace-nowrap">
