            ...
            <div className="bg-white dark:bg-gray-800 rounded-lg shadow-md p-6">
              ...
              <div className="space-y-4">
                {otherAdmins.map((admin) => (
                  <div key={admin._id} className="flex items-center border-b border-gray-200 dark:border-gray-700 pb-3 last:border-0 last:pb-0">
                    ...
                    {adminLogs.map((log) => (
                      <tr key={log.id || `log-${log.timestamp}`}>
                        ...
                        <td className="px-6 py-4 text-sm text-gray-700 dark:text-gray-300">
                          <span className="font-medium">{log.targetType || 'Action'}</span>: {log.description || log.details || 'No details available'}
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap">
                          <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${
                            log.status === 'success' ? 'bg-green-100 text-green-800 dark:bg-green-900 dark:text-green-300' :
                            log.status === 'error' ? 'bg-red-100 text-red-800 dark:bg-red-900 dark:text-red-300' :
